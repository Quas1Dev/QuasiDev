...
**Installer (instalador):** baixa um instalador, que é um programa que prepara outro programa para ser executado no computador.  Ele apresenta uma série de etapas para você customizar os itens que serão instalados junto ao NetBeans, escolher onde colocar os binários, escolher a versão do <abbr title="Java Development Kit">JDK</abbr> que será utilizado, e talvez configurar alguma outra característica do NetBeans de antemão. Além disso, o instalador cria um ícone na área de trabalho para que possa acessar o programa, o que você tem que fazer manualmente se baixar apenas os binários. Enfim, ele auxilia nas configurações que teria de fazer manualmente, caso precisasse.
