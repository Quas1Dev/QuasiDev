...
O principal objetivo do Jekyll é automatizar a criação de múltiplas páginas, permitindo a criação de sites enormes sem que o desenvolvedor precise codificar manualmente cada uma de sua páginas. A utilização de templates é fundamental para esse propósito. 

Template é uma palavra do inglês que denota uma **forma,** **molde ou padrão** usado como um guia para fazer alguma coisa, como a página de um site. Durante esse texto estaremos usando as palavras template e molde como iguais.

Usa-se um programa, que geralmente é referido como processador de modelo, para combinar esse template com dados para gerar uma ou mais páginas. Esse programa entende uma linguagem denominada linguagem modelo, que é usada para orientar o gerador no preenchimento do template. A partir desses comandos, o gerador decide como preencher o template para gerar a página em questão.
...
Shopify é uma plataforma de comércio eletrônico (do inglês e-commerce).

<dfn>Liquid</dfn> é uma **linguagem de modelo** criada pela Shopify para escrever os templates usados na criação de sites de compra em sua plataforma. 
