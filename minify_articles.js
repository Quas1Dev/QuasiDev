...
let code = fs.readFileSync('_css/sources/article-style.css','utf8');
...
fs.writeFile('_css/article.css.map', minified.map.toString(), (err) => {
    if (err) { throw err }
});

fs.writeFile('_css/article-style.min.css',minified.css+'/*# sourceMappingURL=article.css.map*/', (err)=>{
	if (err) {throw err}
})
